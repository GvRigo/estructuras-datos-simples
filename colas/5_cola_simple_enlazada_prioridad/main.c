    ...
    do {
        ...
        switch (opcion) {
            case 1:  // agregar nodo
                ...
                prioridad = lee_numero("\n\tIntroduce prioridad para el nodo: ");
                limpiar_pantalla();
                Nodo *nuevo_nodo = crear_nodo(id, prioridad);
                if (nuevo_nodo == NULL) {
                    printf("\n\t***NO SE PUDO CREAR EL NODO***\n\n");                
                    return 1;
                } else {
                    insertar_nodo_prioridad(&cola, nuevo_nodo);
            ...
            case 5:  // recuperar nodo_extraido
                if (nodo_extraido != NULL) {
                    limpiar_pantalla();
                    insertar_nodo_prioridad(&cola, nodo_extraido);
